...
def require_role(db, user_id: UUID, allowed_roles: list[str]) -> None:
    ...
    if not any(role in allowed_roles for role in user_roles):
        raise HTTPException(
            status_code=403, detail={"error": "forbidden", "message": "Insufficient permissions"}
        )


@router.post("/sensitive-access/request", response_model=SensitiveAccessResponse, status_code=201)
async def request_sensitive_access(
    request: Request, body: SensitiveAccessRequestBody, current_user: CurrentUser, db: DBSession
) -> SensitiveAccessResponse:
    """Request access to sensitive data (SECRETARY only)."""
    require_role(db, current_user.id, ["SECRETARY", "DEV"])

    target_profile = (
        db.query(UserProfile).filter(UserProfile.user_id == body.target_user_id).first()
    )
    if not target_profile:
        raise HTTPException(
            status_code=404, detail={"error": "not_found", "message": "Target user not found"}
        )
    ...
    if existing:
        raise HTTPException(
            status_code=409,
            detail={"error": "conflict", "message": "Pending request already exists"},
        )
    ...
    create_audit_log(
        ...
        entity_type="sensitive_access_request",
        entity_id=str(access_request.id),
        ip=request.client.host if request.client else None,
        user_agent=request.headers.get("user-agent"),
        metadata={"target_user_id": str(body.target_user_id), "scope": "CPF_RG"},
    )
    db.commit()
...
async def get_pending_access_requests(
    ...
    requests = (
        db.query(SensitiveAccessRequest)
        .filter(SensitiveAccessRequest.status == "PENDING")
        .order_by(SensitiveAccessRequest.created_at.desc())
        .all()
    )
    return [
        SensitiveAccessResponse(
...
async def approve_sensitive_access(
    ...
    create_audit_log(
        ...
        metadata={
            ...
            "target_user_id": str(access_request.target_user_id),
...
async def reject_sensitive_access(
    ...
    create_audit_log(
        db=db,
        actor_user_id=current_user.id,
        action="sensitive_access_rejected",
        entity_type="sensitive_access_request",
        entity_id=str(request_id),
        ip=request.client.host if request.client else None,
        user_agent=request.headers.get("user-agent"),
...
async def get_user_documents(
    request: Request, user_id: UUID, current_user: CurrentUser, db: DBSession
) -> DocumentsResponse:
    ...
    if not is_dev:
        ...
        if not access_request:
            raise HTTPException(
                status_code=403,
                detail={"error": "forbidden", "message": "No valid access permission"},
            )

    profile = db.query(UserProfile).filter(UserProfile.user_id == user_id).first()
    if not profile or not profile.cpf_encrypted or not profile.rg_encrypted:
        raise HTTPException(
            status_code=404, detail={"error": "not_found", "message": "Documents not found"}
        )

    if not crypto_service.is_configured:
        raise HTTPException(
            status_code=503,
            detail={"error": "service_unavailable", "message": "Decryption service not available"},
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail={"error": "decryption_error", "message": "Failed to decrypt documents"},
        )
    ...
    create_audit_log(
        ...
        metadata={
            ...
            "request_id": str(access_request.id) if access_request else None,
        },
    )
    db.commit()
