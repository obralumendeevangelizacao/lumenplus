...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE lifedimension AS ENUM ('HUMANA', 'ESPIRITUAL', 'COMUNITARIA', 'INTELECTUAL', 'APOSTOLICA')"
    )
    op.execute(
        "CREATE TYPE lifemassfrequency AS ENUM ('DAILY', 'WEEKLY_MANY', 'WEEKLY', 'BI_WEEKLY', 'MONTHLY')"
    )
    op.execute(
        "CREATE TYPE lifeconfessionfrequency AS ENUM ('WEEKLY', 'BI_WEEKLY', 'MONTHLY', 'OTHER')"
    )
    op.execute(
        "CREATE TYPE lifereviewdecision AS ENUM ('CONTINUE', 'ADJUST_GOAL', 'CHANGE_PRIMARY_GOAL', 'NEW_CYCLE')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "cycle_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("life_plan_cycles.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "goal_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("life_plan_goals.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_index(
        "idx_life_plan_monthly_reviews_cycle_id", "life_plan_monthly_reviews", ["cycle_id"]
    )
