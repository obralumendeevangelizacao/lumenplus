...
def _retreat_to_dict(
    ...
    if registration:
        result["my_registration"] = {
            ...
            "fee_label": FEE_CATEGORY_LABELS.get(
                registration.fee_category, registration.fee_category
            )
            if registration.fee_category
            else None,
            "assigned_house_id": str(registration.assigned_house_id)
            if registration.assigned_house_id
            else None,
            "notes": registration.notes,
            "payment_proof_url": registration.payment_proof_url,
            "payment_submitted_at": registration.payment_submitted_at.isoformat()
            if registration.payment_submitted_at
            else None,
            "payment_confirmed_at": registration.payment_confirmed_at.isoformat()
...
def _spots_available(db, retreat: Retreat, modality: str | None = None) -> bool:
    ...
    if modality and retreat.houses:
        ...
        modality_capacity = sum(
            h.max_participants for h in modality_houses if h.max_participants is not None
        )
...
async def get_retreat(retreat_id: UUID, current_user: CurrentUser, db: DBSession):
    ...
    if not as_participant and not as_service:
        raise HTTPException(
            status_code=403,
            detail={"error": "forbidden", "message": "Você não é elegível para este retiro"},
        )
...
async def register_for_retreat(
    ...
    if not retreat or retreat.status != RetreatStatus.PUBLISHED:
        raise HTTPException(
            status_code=404,
            detail={"error": "not_found", "message": "Retiro não encontrado ou não está aberto"},
        )
    ...
    if body.registration_type == "SERVICE":
        if not _user_eligible_as_service(db, current_user.id, retreat):
            raise HTTPException(
                ...
                detail={
                    "error": "forbidden",
                    "message": "Você não é elegível para a equipe de serviço deste retiro",
                },
            )
    else:
        ...
        if len(available_modalities) > 1 and not modality:
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": "Selecione a modalidade de participação",
                },
            )
        if modality and modality not in available_modalities:
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": f"Modalidade '{modality}' não disponível neste retiro",
                },
            )
    ...
    if existing and existing.status != RegistrationStatus.CANCELLED:
        raise HTTPException(
            status_code=409,
            detail={"error": "already_registered", "message": "Você já está inscrito neste retiro"},
        )
        ...
        for order, team_id in enumerate(prefs_to_save, start=1):
            ...
            if not team or team.retreat_id != retreat_id:
                raise HTTPException(
                    ...
                    detail={
                        "error": "invalid_team",
                        "message": f"Equipe {team_id} não pertence a este retiro",
                    },
                )
            db.add(
                RetreatTeamPreference(
...
async def submit_payment_proof(
    ...
    if reg.status == RegistrationStatus.CONFIRMED:
        raise HTTPException(
            status_code=409,
            detail={"error": "already_confirmed", "message": "Pagamento já confirmado"},
        )
    ...
    if (
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": "Serviço de armazenamento não configurado. Contate o administrador.",
            },
        )
    ...
    return {
        "message": "Comprovante enviado com sucesso. Aguarde a confirmação do administrador.",
