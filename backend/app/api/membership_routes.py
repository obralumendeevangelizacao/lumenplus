...
def has_global_role(db, user_id: UUID, role_codes: list[str]) -> bool:
    return (
        ...
        .join(GlobalRole)
        .filter(UserGlobalRole.user_id == user_id, GlobalRole.code.in_(role_codes))
        .first()
        is not None
    )
...
async def request_membership(
    request: Request, body: MembershipRequestBody, current_user: CurrentUser, db: DBSession
) -> MembershipResponse:
    """Request membership to an organization unit."""
    org_unit = db.query(OrgUnit).filter(OrgUnit.id == body.org_unit_id, OrgUnit.is_active).first()
    if not org_unit:
        raise HTTPException(
            status_code=404, detail={"error": "not_found", "message": "Organization unit not found"}
        )

    existing = (
        db.query(OrgMembership)
        .filter(
            OrgMembership.user_id == current_user.id, OrgMembership.org_unit_id == body.org_unit_id
        )
        .first()
    )
    if existing:
    ...
    create_audit_log(
        ...
        metadata={"org_unit_id": str(body.org_unit_id)},
    )
    db.commit()
...
async def get_pending_members(
    org_unit_id: UUID, current_user: CurrentUser, db: DBSession
    ...
    if not (
        ...
        or is_coordinator_of(db, current_user.id, org_unit_id)
    ):
        raise HTTPException(
            status_code=403,
...
async def approve_membership(
    request: Request, membership_id: UUID, current_user: CurrentUser, db: DBSession
) -> MembershipResponse:
    ...
    if not (
        has_global_role(db, current_user.id, ["DEV", "COUNCIL_GENERAL"])
        or is_coordinator_of(db, current_user.id, membership.org_unit_id)
    ):
        raise HTTPException(
            status_code=403,
            detail={"error": "forbidden", "message": "Not authorized to approve memberships"},
        )
